...
def train_model(model, train_loader, val_loader, criterion, optimizer, scheduler, device, num_epochs=100, start_epoch=0, patience=10, log_interval=20, steps_per_epoch=None):
    """
    모델 훈련을 위한 메인 루프.
    ...
    for epoch in range(start_epoch, num_epochs):
        ...
        for step, (inputs, labels) in enumerate(train_loader):
            ...
            if model._get_name() == "EmoNet":
                with autocast(device_type=device.type, enabled=use_amp):
                    # 모델의 출력이 딕셔너리이므로 변수명을 _dict로 변경하여 명확화
                    outputs_dict = model(inputs)
                    # 딕셔너리에서 'expression' 키로 감정 예측 텐서를 직접 추출
            ...
            if model._get_name() == "EmoNet":
                _, preds = torch.max(emotion_preds, 1)
            else:
