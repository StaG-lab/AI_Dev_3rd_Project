...
class EmoNet(nn.Module):
    def __init__(self, num_modules=2, num_classes=7, n_expressions=8, n_reg=2, n_blocks=4, attention=True, temporal_smoothing=False):
        ...
        self.emo_fc_2 = nn.Sequential(nn.Linear(256, 128), nn.BatchNorm1d(128), nn.ReLU(inplace=True), nn.Linear(128, self.n_expressions + n_reg))

    def forward(self, x, reset_smoothing=False):
        #Resets the temporal smoothing
        if self.init_smoothing:
            self.init_smoothing = False
            self.temporal_state = torch.zeros(x.size(0), self.n_temporal_states, self.n_expressions+self.n_reg).cuda()              
        if reset_smoothing:
